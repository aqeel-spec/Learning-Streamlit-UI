...
# # # CREATE OBJECT OF DATA ANALYZER BOT
...
assistant = st.sidebar.selectbox(
    "Select an Assistant", ("Data Analyst", "Math Tutor","Function Calling","Knowledge Retrieval")
)
...
########################## App functions playlists ###########################


FMP_API_KEY=""

if assistant == "Data Analyst":
    st.header(assistant)
    assistant_id = "asst_1"
    st.header(assistant)
    st.session_state.start_chat = True
    # Create a sidebar for API key configuration and additional features
    st.sidebar.header("Configuration FMP api")
    fmp_api_key = st.sidebar.text_input("Enter your FMP API key", type="password")
    if api_key and fmp_api_key:

        # Initialize the OpenAI client (to ensure that the api key is in sidebar of app)
        # st.session_state.bot[0].OPENAI_API_KEY = api_key
        # st.session_state.bot[0].FMP_API_KEY = fmp_api_key
        # st.sidebar.success("API key set!")
        ...
        st.sidebar.error("Please enter your FMP_API_KEY API key")
        
# elif assistant == "Quiz App":
#     assistant_id = "asst_2"
#     if len(st.session_state.messages) == 0:
#         st.header(assistant)
#         st.write("This app was created using assistant function calling.")
#     st.session_state.start_chat = True
elif assistant == "Math Tutor":
    st.header(assistant)
    thread: Thread  = client.beta.threads.create()
    st.session_state.thread_id = thread.id
    if len(st.session_state.messages) == 0:
        st.header(assistant)
        st.subheader("This is an  example of assisatant code-interpreter")
    st.session_state.start_chat = True
    # st.header(assistant)
elif assistant == "Function Calling":
    ...
    if len(st.session_state.messages) == 0:
        ...
        with tab1:
            html_code = f"""
                <div  style='border:1px solid #e6e6e6; padding: 10px; border-radius: 10px;cursor: pointer'>
                    <div style="color: black; font-weight: bold ">Get developer details</div>
                    <p style="color:"#40414F"; opacity:0.5% ">Function : get_developer_details</p>
                    <p style="color:"#40414F"; opacity:0.5% ">Example : What is the name of the developer?</p>
        ...
        with tab2:
            html_code = f"""
                <div  style='border:1px solid #e6e6e6; padding: 10px; border-radius: 10px; cursor: pointer'>
                    <div style="color: black;fontsize: 14px; font-weight: bold;  ">Get the current weather in a given location</div>
                    <p style="color:"#40414F"; opacity:0.5% "> Function : get_current_weather </p>
                    <p style="color:"#40414F"; opacity:0.5% "> Available Location : Tokyo, San Francisco, Paris </p>
                    <p style="color:"#40414F"; opacity:0.5% "> Example : What is the current weather in Paris? </p>
...
def data_analyzer_bot(prompt):
    # st.write(st.session_state.bot[0].OPENAI_API_KEY)
    # st.write(st.session_state.bot[0].FMP_API_KEY)
    if prompt != '':
        # st.session_state.messages.append({"role": "user", "content": prompt})
        # with st.chat_message("user"):
        #     st.markdown(prompt)
        st.session_state.bot[0].send_message(prompt)
        
       
        # get latest response from bot
        messages = st.session_state.bot[0].getMessages()
        print("st.session_state.bot[0] =>",st.session_state.bot[0])
        for m in st.session_state.bot[0].getMessages():
            st.session_state.messages.append({"role": "user", "content": m})
            with st.chat_message(m.role):
                st.markdown(m.content)
        
        # Call the function to initiate progress and processing:
        ...
        with st.spinner("Analyzing data and preparing results for you ..."):
            time.sleep(10)
            # st.success("Processing complete!")
       
        st.session_state.bot[0].isCompleted()
        if st.session_state.bot[0].isCompleted() == True:
            messages = st.session_state.bot[0].get_lastest_response()
                ...
                if message.role == "assistant" :
                    for content in message.content:
                        if content.type == "text":
                            ...
                            st.session_state.messages.append({"role": message.role, "content": message_content})
                            with st.chat_message(message.role):
                                st.markdown(message_content)
                        elif content.type == "image_file":
                            image_file_id = content.image_file.file_id
                            image_save_path = f"output_images/image_{image_file_id}.png"
                            download_and_save_image(image_file_id, image_save_path)  # Pass save path
                            st.image(image_save_path, width=200)  # Use st.image for display
...
if st.session_state.start_chat:
    ...
    if prompt := st.chat_input("What is up?"):
        ...
        elif assistant == "Data Analyst":
            data_analyzer_bot(prompt=prompt)
        else:
